...
const criarCaixaTelefone = (numeroTelefone, idTelefone) => {
    ...
    img.addEventListener('click', (evt) => {
        ...
        const endpoint_delTelefone = `http://127.0.0.1:1880/deletarcolaboradorporid/${idtel}`;
...
f_telefone.addEventListener('keyup', (evt) => {
    if (evt.key == 'Enter') {
        if (evt.target.value.length >= 8) {
            criarCaixaTelefone(evt.target.value)
            evt.target.value = '';
        } else {
...
const pegarColaboradorPorId = (id) => {
    ...
    const endpoint_TelefoneColaboradorPorId = `http://127.0.0.1:1880/telefonescolaboradorporid/${id}`;
    fetch(endpoint_TelefoneColaboradorPorId)
        .then(res => res.json())
        .then(res => {
            telefones.innerHTML = '';
            res.forEach(el => {
                criarCaixaTelefone(el.s_numero_telefone, el.n_telefone_telefone);
            });
        })
}