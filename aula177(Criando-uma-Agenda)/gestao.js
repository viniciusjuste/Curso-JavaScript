            ...
            if (Array.isArray(res) && res.length > 0) {
                res.forEach((el) => {
                    ...
                    img_editar.addEventListener('click', (evt) => {
                        ...
                        f_dataNascimento.value = formatarData(dados[4].innerHTML)
                        console.log(dados)
                    })
...
const btn_gravar = document.getElementById('btn_gravar').addEventListener('click', (evt) => {
    removerOculto.classList.add('ocultar')
    const endpoint = `http://127.0.0.1:1880/atualizarcontatos/${f_id.value}/${ f_nome.value}/${f_celular.value}/${f_email.value}/${f_dataNascimento.value}`
    fetch(endpoint)
    .then(res => {
        if(res.status == 200){
    ...
    f_celular.value = dados[2].innerHTML
    f_email.value = dados[3].innerHTML
    f_dataNascimento.value = dados[4].innerHTML
})
