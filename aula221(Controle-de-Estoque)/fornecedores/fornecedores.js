...
const endpoint = `${servidor}tiposfornecedor`;
fetch(endpoint)
    .then(res => res.json())
    .then(res => {
        console.log(res);
        ...
        res.forEach(el => {
            ...
            option.setAttribute('value', el.n_tipopessoa_tipopessoa);
            option.innerHTML = el.s_desc_tipopessoa;
    ...
    fetch(endpointNovoEditarColab, cabecalho)
        .then(res => {
            if (res.status == 200) {
                if (modoJanela == 'n') {
                    const config = {
                        titulo: 'Novo colaborador gravado',
                        texto: 'colaborador gravado com sucesso.',
                        cor: '00f',
                        tipo: 'ok',
                        ok: () => {
                            console.log('Ok clicado');
