...
btn_pesquisar.addEventListener('click', () => {
    ...
    if (f_pesq.value != '') {
        const endpointPesq = `${servidor}pesquisaProd/${tipo}/${f_pesq.value}`;
        fetch(endpointPesq)
            .then(res => res.json())
            .then(res => {
                console.log(res);
