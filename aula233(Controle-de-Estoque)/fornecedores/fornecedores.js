...
const telefonesContForn = document.getElementById('telefonesContForn');
const btn_fechartelefonesContForn = document.getElementById('btn_fechartelefonesContForn');
const dadosGrid_telefonesContForm = document.getElementById('dadosGrid_telefonesContForm');
...
const addContForn = (id, nome) => {
    const linhaGrid = document.createElement('div');
    linhaGrid.setAttribute('class', 'linhaGrid novoContForn');
    ...
    img_verFoneContForn.addEventListener('click', (evt) => {
        const id = evt.target.parentNode.parentNode.firstChild.innerHTML;
        const endpoint = `${servidor}retornaTelefones/${id}`;
        fetch(endpoint)
            .then(res => res.json())
            .then(res => {
                telefonesContForn.classList.remove('ocultar_popUp');
                dadosGrid_telefonesContForm.innerHTML = '';
                const mzi = maiorZIndex() + 2;
                telefonesContForn.setAttribute('style', `z-index: ${mzi} !important`);
                res.forEach(el => {
                    addTelefoneContForn(el.s_numero_telefone);
                });
            });
    });
...
const addTelefoneContForn = (telefone) => {
    ...
    const colunaLinhaGrid2 = document.createElement('div');
    colunaLinhaGrid2.setAttribute('class', 'colunaLinhaGrid c2_lcf');
    colunaLinhaGrid2.innerHTML = telefone;
    linhaGrid.appendChild(colunaLinhaGrid2);

    dadosGrid_telefonesContForm.appendChild(linhaGrid);
}
...
btn_fechartelefonesContForn.addEventListener('click', (evt) => {
    telefonesContForn.classList.add('ocultar_popUp');
});
...
btn_gravarPopUp.addEventListener('click', (evt) => {
    ...
    contat.forEach(el => {
        a_contat.push(el.firstChild.innerHTML);
    });
